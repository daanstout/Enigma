...
int main(){
  ...
  while(1){
    ...
    if(strncmp(command, "rotateForward", 13) == 0){           //als het commando van DE2 rotateForward is
      slaveSendCommand(command[strlen(command) - 1] - 37, "rotateForward/");
    }else if(strncmp(command, "getRotorLetter", 14) == 0){
      slaveSendCommand(command[strlen(command) - 1] - 37, "getRotorLetter/");
      result[0] = slaveReceiveCommand(command[strlen(command) - 1] - 37);
      result[1] = '/';
      myUsart.println(result);                                //stuurt de byte naar het DE2 board
    }else if(strncmp(command, "rotateBackwards", 15) == 0){
      slaveSendCommand(command[strlen(command) - 1] - 37, "rotateBackwards/");
    }else if(strncmp(command, "getLetter", 9) == 0){
      slaveSendCommand(command[strlen(command) - 2] - 37, "getLetter", command[strlen(command) - 1]);
      result[0] = slaveReceiveCommand(command[strlen(command) - 2] - 37);
      result[1] = '/';
      myUsart.println(result);
      //PORTB ^= (1 << PORTB5);
    }else if(strncmp(command, "getReverseLetter", 16) == 0){
      slaveSendCommand(command[strlen(command) - 2] - 37, "getReverseLetter", command[strlen(command) - 1]);
      result[0] = slaveReceiveCommand(command[strlen(command) - 2] - 37);
      result[1] = '/';
      myUsart.println(result);
    }else if(strncmp(command, "changeRingCfg", 13) == 0){
      slaveSendCommand(command[strlen(command) - 2] - 37, "changeRingCfg", command[strlen(command) - 1]);
    }else if(strncmp(command, "getTriggered", 12) == 0){
      PORTB ^= (1 << PORTB5);
      slaveSendCommand(command[strlen(command) - 1] - 37, "getTriggered/");
      result[0] = slaveReceiveCommand(command[strlen(command) - 1] - 37) + 48;
      result[1] = '/';
      myUsart.println(result);
...
char slaveReceiveCommand(int adres){
  _delay_ms(20);                   //wacht 15 millieseconden om de slave zijn werk te laten doen
